...
public class Diccionario<K, V> implements Iterable<V> {
     ...
     */
     public Diccionario(int capacidad, Dispersor<K> dispersor) {
           ...
           capacidad = capacidad < MINIMA_CAPACIDAD ? nuevaCapacidad(MINIMA_CAPACIDAD) : nuevaCapacidad(capacidad);
           this.entradas = nuevoArreglo(capacidad);
     }

//Metodo privado me regresa la nuevaCapacidad del arreglo
...
//Metodo privado aumenta la capacidad del arreglo
    private void aumenta(){
        int n=(entradas.length*2);
        Lista<Entrada>[] copia=entradas;
        entradas=nuevoArreglo(n);
        elementos=0;
        for (int i=0; i< copia.length; i++){
     	      if(copia[i] != null){
     		       for (Entrada e : copia[i]){
...
//Metodo privado regresa la dispersion de lallave
